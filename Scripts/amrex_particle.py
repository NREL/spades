...
class AmrexParticleFile:
    ...
    def load_binary_data(self):
        ...
        self.real_data = np.empty(
            (self.num_particles, self.ndim + self.num_reals), dtype=float
        )
        self.int_data = np.empty((self.num_particles, 2 + self.num_ints), dtype=int)
        ...
        for lev, ginfo in enumerate(self.grid_info):
            ...
            for idx, npts, offset in ginfo:
                ...
                for i, ii in enumerate(range(0, nints * npts, nints)):
                    ioffset = i * nints
                    for jj in range(nints):
                        idata[pidx, jj] = ivals[ioffset + jj]

                    roffset = i * nreals
                    for jj in range(nreals):
                        rdata[pidx, jj] = rvals[roffset + jj]
                    pidx += 1

            for fh in bfiles.values():
                fh.close()

        int_var_names = ["id", "cpu"] + self.int_var_names
        rvar_names = [f"x{i}" for i in range(self.ndim)] + self.real_var_names
        idict = {key: idata[:, i] for i, key in enumerate(int_var_names)}

...
def make_lattice(nx, Ny, nz, f_vacant):
    ...
    cutoff_list = [0.4 * spacing / np.sqrt(2.0) for i in range(len(lattice))]
    # print(cutoff_list)
        ...
        for j in indices:
            if (
                ...
                if nneighbors > 1:  # Site j is a candidate for the vacancy to go here
                    ...
                    boltzmann = np.exp(
                        beta * (site_i_energy)
                    )  # Minus sign is exp(-beta*-site_i_energy)
...
def kmc_find_site(propensity, kmc_algorithm="rfKMC"):
    ...
    if kmc_algorithm == "R_KMC":
        ...
        prob = (
            propensity[enumber] / rmax
        )  # This is a made up probability - there are other ways to set the probability (e.g., an energy criterion) so this is a placeholder to test the R_KMC functionality
...
def do_event_at_site(
    ...
    for site in unique_update_list:
        psum, events = build_site_events(
            site, lattice, nl, ratelist, beta, nn_delta_e, ndim
        )
...
def main():
    ...
    parser.add_argument("-p", "--do_plots", help="Make plots", action="store_true")
    parser.add_argument("-s", "--save_plots", help="Save plots", action="store_true")
    ...
    print(
        "The site spacing is {} cm, so this corresponds to D = rate*d^2 m^2/s = {} for vacancies".format(
            spacing, spacing * spacing * hop_rate_not
        )
    )
